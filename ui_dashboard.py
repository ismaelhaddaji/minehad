...
with st.sidebar:
    ...
    api_key = st.text_input(
        ...
        value="demo",
        type="password",
        help="Enter your Alpha Vantage API key. Leave 'demo' for synthetic data."
    )
    ...
    risk_pct = st.slider("Risk Per Trade (%)", min_value=0.5, max_value=3.0, value=1.0, step=0.5) / 100
    min_rr = st.select_slider("Min Risk:Reward", options=[1.5, 2.0, 2.5, 3.0], value=2.0)
...
def load_data(api_key: str, primary_tf: str):
    ...
    timeframes = fetch_all_timeframes(api_key)
    ...
    return timeframes, features, current_price
...
def run_full_analysis(api_key: str, primary_tf: str):
    ...
    timeframes, features, current_price = load_data(api_key, primary_tf)
